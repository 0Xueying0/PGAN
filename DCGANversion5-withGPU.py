...
#        out = torch.cat((A, out),axis=0)
        ...
        out = torch.Tensor(np.concatenate((out.cpu().detach().numpy(), y.cpu().detach().numpy()), axis=1))
        if train_on_gpu:
            out = out.cuda()
...
#        print("fake_series1",fake_series1.size())
    ...
    x = torch.Tensor(x_test[:batch_size,:,:,:])
    if train_on_gpu:
        fixed_z = fixed_z.cuda()
        part_realseries = part_realseries.cuda()
        x = torch.Tensor(x_test[:batch_size,:,:,:]).cuda()
    samples_z = G(x,y=fixed_z)
...
t = np.concatenate((x_test[0,0,0,:],np.array(samples_z.cpu().detach().numpy())[0,0,0,:]),0)
...
x=torch.Tensor(x_test[:batch_size,:,:,:])
if train_on_gpu:
    fixed_z1 = fixed_z1.cuda()
    x =x.cuda()
samples_z = G(x,y=fixed_z1)
if train_on_gpu:
...
for j in range(29):
    ...
    x=list(range(57))
    plt.plot(x[:41],t[:41], label='past price',alpha=0.5)
    plt.plot(x[40:],t[40:], label='simulation price',alpha=0.5)
    plt.plot(x[40:],y_test[0,0,j,:], label='real price',alpha=0.5)
...
for j in range(29):
    title=['AAPL.O','AXP.N','BA.N','CAT.N','CSCO.O','CVX.N','DIS.N','GS.N','HD.N','IBM.N','INTC.O','JNJ.N','JPM.N','KO.N','MCD.N','MMM.N','MRK.N','MSFT.O','NKE.N','PFE.N','PG.N','TRV.N','UNH.N','UTX.N','V.N','VZ.N','WBA.O','WMT.N','XOM.N']
    t = np.concatenate((x_test[0,0,j,:],np.array(samples_z.cpu().detach().numpy())[0,0,j,:]),0)
    p = t*data_test[0,0,j,:]+data_test[0,0,j,:]
    real = data_test[0,0,j,:]
    x=list(range(57))

    plt.plot(x[:41],p[:41], label='past data', alpha=0.5)
    plt.plot(x[40:],p[40:], label='simulation data',alpha=0.5)
    plt.plot(x[40:],real[40:], label='real data',alpha=0.5)
    #plt.plot(samples, label='Generator', alpha=0.5)
    plt.title(title[j])
    plt.legend()
    figname = 'fig_{}.png'.format(j)
    dest = os.path.join(path, figname)
    plt.savefig(dest)  # write image to file
    plt.clf()
