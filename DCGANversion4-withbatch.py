...
#    for idx in range(50):
        print('Epoch: ', epoch, '| Step: ', idx, '| batch x: ',
                  part_realseries.size(), '| batch y: ', y_realseries.size())

#        part_realseries = np.array(part_realseries)
#        y_realseries = np.array(y_realseries)
#        part_realseries = np.apply_along_axis(scale, 3, part_realseries)
#        part_realseries=torch.squeeze(torch.Tensor(part_realseries),2)
#        y_realseries = np.apply_along_axis(scale, 3, y_realseries)
#        y_realseries=torch.squeeze(torch.Tensor(y_realseries),2)
...
#    samples_z = G(x=torch.Tensor(np.array(x_test[:8,:,:,:])),y=fixed_z1)
